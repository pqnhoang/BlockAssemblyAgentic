...
def add_link(robot_root, link_name, link_definition: SMLinkDefinition, origin):
    ...
    vis_box = ET.SubElement(
        vis_geom, link_definition.shape_type, link_definition.visual_shape_dimensions_dict
    )
    ...
    col_box = ET.SubElement(
        col_geom, link_definition.shape_type, link_definition.visual_shape_dimensions_dict
    )
...
def create_cmassembly_urdf(
    ...
    for manip_nr, manipulator_offset_pair in enumerate(manipulator_definition_pairs):
        ...
        for actuator_definition, actuator_nr in zip(
            ...
            for segment_nr in range(actuator_definition.n_segments + 1):

                # ==========================================================
                # === DÁN TOÀN BỘ NỘI DUNG MỚI NÀY VÀO TRONG VÒNG LẶP ===
                # ==========================================================

                # --- Xác định joint cho đốt hiện tại (logic cũ) ---
                if total_segment_counter > 0:
                ...
                # --- Đặt tên (logic cũ) ---
                segment_name = f"Man{manip_nr}Act{actuator_nr}Seg{segment_nr}"
                parent_name = child_name
                child_name = segment_name
                if total_segment_counter > 0:
                    joint_name = f"{parent_name}_to_{child_name}{ax_str}"

                # --- Lấy định nghĩa link (logic cũ) ---
                if actuator_definition.link_definition.shape_type in ["box", "cylinder", "sphere"]:
                    link_to_add = copy.copy(actuator_definition.link_definition)
                elif actuator_definition.link_definition.shape_type == "stadium":
                    link_to_add = copy.copy(actuator_definition.link_definition)
                    link_to_add.shape_type = "box"
                elif actuator_definition.link_definition.shape_type == "capsule":
                    link_to_add = copy.copy(actuator_definition.link_definition)
                    link_to_add.shape_type = "cylinder"
                
                # --- LOGIC MỚI ĐỂ LẮP RÁP ---
                if segment_nr == 0:
                    # Logic mới, tường minh cho đốt đầu tiên
                    link_to_add.reduce_height(0.5)
                    joint_to_add = SMJointDefinition(joint_type="fixed")
                    joint_name = f"{parent_name}_to_{child_name}"

                    # 1. Tạo link với origin (0,0,0) để hình học không bị lệch
                    add_link(robot, child_name, link_to_add, origin=[0, 0, 0, 0, 0, 0])
                    
                    # 2. Tạo joint nối base với link mới, dùng trực tiếp offset bạn đã tính
                    add_joint(
                        robot,
                        joint_name,
                        parent_name, # parent_name lúc này là base_link
                        child_name,
                        joint_to_add,
                        origin=manipulator_offset,
                    )
                else:
                    # Logic cũ cho các đốt còn lại
                    if segment_nr == actuator_definition.n_segments:
                        link_to_add.reduce_height(0.5)
                ...
                # --- Xử lý các hình dạng phức tạp (stadium/capsule) - giữ nguyên logic cũ ---
                if (not segment_nr == 0 and actuator_definition.link_definition.shape_type in ["stadium", "capsule"]):
                    # ... (dán lại toàn bộ khối if xử lý stadium/capsule ở đây) ...
                    # Bắt đầu từ dòng "if actuator_definition.link_definition.shape_type in ["stadium"]:"
                    # đến hết lệnh gọi add_joint_link_pair cuối cùng bên trong nó.
                    if actuator_definition.link_definition.shape_type in ["stadium"]:
                        ...
                        if joint_to_add.axis == [1, 0, 0]:
                            ...
                            dim = [actuator_definition.link_definition.dimensions[0], actuator_definition.link_definition.dimensions[1] / 2.0]
                        elif joint_to_add.axis == [0, 1, 0]:
                            additional_link_origin_rotation = [0, 0, 0, np.pi / 2, 0, 0]
                            dim = [actuator_definition.link_definition.dimensions[1], actuator_definition.link_definition.dimensions[0] / 2.0]
                        else:
                            assert False
                        ...
                        helper_shape_height = 2 * actuator_definition.link_definition.dimensions[1]
                    
                    helper_offset = [x - y - z for x, y, z in zip(actuator_definition.link_definition.origin_offset, additional_link_origin_rotation, [0, 0, helper_shape_height / 2.0, 0, 0, 0])]
                    additional_link_to_add = SMLinkDefinition(shape_type=helper_shape, dimensions=dim, mass=actuator_definition.link_definition.mass / 1000.0, inertial_values=[x / 1000.0 for x in actuator_definition.link_definition.inertial_values], material_color=actuator_definition.link_definition.material_color, material_name=actuator_definition.link_definition.material_name, origin_offset=helper_offset)
                    additional_joint_to_add = SMJointDefinition(joint_type="fixed")
                    add_joint_link_pair(robot_root=robot, parent_link_name=parent_name, child_link_name=child_name + "helper_shape", joint_name=joint_name + "helper_shape", link=additional_link_to_add, joint=additional_joint_to_add, previous_link=previous_link, previous_joint=previous_joint)
                
                # --- Cập nhật cho vòng lặp tiếp theo (logic cũ) ---
