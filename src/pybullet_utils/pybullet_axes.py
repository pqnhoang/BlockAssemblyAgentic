...
def add_axes_to_image(
    ...
    # Draw the canvas and convert it to a NumPy array - THIS IS THE FIXED PART
    canvas = FigureCanvas(fig)
    canvas.draw()
    
    # Thay thế phương thức tostring_rgb() với buffer_rgba()
    buf = canvas.buffer_rgba()
    image_with_axes = np.asarray(buf, dtype="uint8")
    
    # Chuyển đổi từ RGBA sang RGB nếu cần
    if image_with_axes.shape[2] == 4:  # Nếu có kênh alpha
        image_with_axes = image_with_axes[:, :, :3]
...
if __name__ == "__main__":
    ...
    AXES_IMG_PATH = os.path.join(BASE_PATH, "imgs/axes.png")
    ...
    from src.structure.structure import Block
