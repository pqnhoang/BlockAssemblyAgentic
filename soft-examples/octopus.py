...
class IsometricImageJoint:
    ...
    def decide_position(self, order, iter=0):
        # --- PHẦN SỬA LỖI ---
        # 1. Tạo ra cấu trúc dữ liệu cho ví dụ trước
        example_json_list = [
            {
                "name": "support1",
                "shape": "cylinder",
                ...
                "color": [0.5, 0.5, 0.5, 1],
                # Tâm của khối trụ cao 40 đặt trên đất (z=0) sẽ ở z=20.
            ...
            },
            {
                ...
                "color": [0.5, 0.5, 0.5, 1],
                # Mặt trên của support ở z=40, tâm của deck cao 20 sẽ ở 40 + (20/2) = 50.
                "position": {"x": 0, "y": 0, "z": 50},
                "yaw": 45,
            },
        ]
        # 2. Chuyển nó thành chuỗi JSON markdown
        example_json_string = markdown_json(example_json_list)
        # --- KẾT THÚC PHẦN SỬA LỖI ---

        prompt = f"""
        With the stacking order determined as {order}, I now need to know the x, y, and z positions, as well as the yaw angle (in degrees), for each block to build a {self.object_name} structure.

        The x and y coordinates should represent the center of each block on the horizontal plane. The z coordinate represents the height of the center of the block from the ground (where the ground is at z=0). The yaw angle refers to the rotation around the z-axis in degrees. Remember, you can swap the dimensions of blocks to adjust their configuration.

        Ensure that blocks at similar heights in the structure are spaced out in x and y so that they don't collide.

        Make sure the structure is roughly centered at the origin (0, 0), and that each block stacks correctly on the specified blocks (or the ground). Every block must have a stable base to prevent it from falling. The z position should be calculated based on the height of the supporting blocks. For a block on the ground, its z position will be half of its own height.

        Consider the dimensions of the blocks when determining the positions. Provide your reasoning for the chosen x, y, z positions and the yaw angle for each block.
    ...
    def make_urdf_and_view(self, manipulator_yaml_path: str):
        """
        Find the base link, create a URDF file, and view it in PyBullet.
        This function assumes that the positions dictionary contains blocks with 'shape' and 'position' keys.
        """

        # 1. Chia các khối thành 2 nhóm: khớp nối (joints) và không phải khớp nối (non-joints)
        joints = []
        non_joints = []
        ...
        # 2. Tự động tìm base link dựa trên "độ phổ biến"
        ...
        # 3. Tạo định nghĩa Base Link cho thư viện SOMO
        ...
        # 4. Tạo các cặp Manipulator và Offset với thuộc tính động (màu sắc, vị trí Z)
        manipulator_template = SMManipulatorDefinition.from_file(manipulator_yaml_path)
        manipulator_pairs = []
        
        # Dùng enumerate trên self.positions để có index duy nhất cho mỗi material
        for i, part in enumerate(self.positions):
            # Chỉ xử lý các bộ phận là 'joint'
            if part.get('shape', '').lower() != 'joint':
                continue

            # Tính toán vị trí và góc tương đối
            relative_pos_m = (part['position_np'] - base_position_np) / 1000.0
            angle = np.arctan2(relative_pos_m[1], relative_pos_m[0])
            
            # Sử dụng tọa độ z động từ `relative_pos_m`
            offset = [relative_pos_m[0], relative_pos_m[1], relative_pos_m[2] + 0.025, 0, np.pi/2, angle]
            
            # Tạo một bản sao độc lập của khuôn mẫu cho joint hiện tại
            new_manipulator_def = copy.deepcopy(manipulator_template)

            # Truy cập đúng vào cấu trúc lồng nhau để thay đổi màu sắc
            try:
                # Lấy màu từ dữ liệu của joint hiện tại
                new_manipulator_def.actuator_definitions[0].link_definition.material_color = part['color']
                # Tạo một tên material độc nhất để tránh xung đột trong file URDF
                new_manipulator_def.actuator_definitions[0].link_definition.material_name = f"joint_material_{i}"
            except (AttributeError, IndexError) as e:
                print(f"Warning: Could not set color for joint '{part['name']}'. Structure might not match. Error: {e}")

            # Thêm bản sao đã được tùy chỉnh vào danh sách
            manipulator_pairs.append((new_manipulator_def, offset))
        ...
        print(f"🎉 URDF is created at: {self.urdf_path}")

        # 6. Hiển thị trong PyBullet (nếu cần)
        self._view_urdf_in_pybullet()
    def _view_urdf_in_pybullet(self):
        if not self.urdf_path:
            print("❌ Urdf cant not find")
            return

        print(f"👀 Openning Pybullet '{os.path.basename(self.urdf_path)}'...")
