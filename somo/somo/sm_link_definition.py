...
class SMLinkDefinition:
    ...
    def load_to_pybullet(self, physicsClient=-1):
        if self.shape_type == "cuboid":
            half_extents = [dim / 2 for dim in self.dimensions]
            collision_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=half_extents)
            visual_shape = p.createVisualShape(
                p.GEOM_BOX, halfExtents=half_extents, rgbaColor=self.material_color
            )
            id = p.createMultiBody(
                ...
                basePosition=self.origin_offset[:3],
                baseOrientation=self.origin_offset[3:],
            )
            p.changeDynamics(
                id, -1, lateralFriction=0.5, spinningFriction=0.2
            )
            return id
        elif self.shape_type == "cylinder":
            collision_shape = p.createCollisionShape(
                p.GEOM_CYLINDER, radius=self.dimensions[0], height=self.dimensions[1]
            )
            visual_shape = p.createVisualShape(
                p.GEOM_CYLINDER, radius=self.dimensions[0], length=self.dimensions[1], rgbaColor=self.material_color
            )
            id = p.createMultiBody(
                baseMass=self.mass,
                baseCollisionShapeIndex=collision_shape,
